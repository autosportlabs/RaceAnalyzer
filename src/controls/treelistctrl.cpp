...
static const int RENAME_TIMER_TICKS = 100; // minimum rename wait time in ms
...
BEGIN_EVENT_TABLE (wxEditSpinCtrl,wxSpinCtrl)
    ...
    EVT_SPINCTRL	   (wxID_ANY, wxEditSpinCtrl::OnSpin)
END_EVENT_TABLE()
...
void wxEditSpinCtrl::OnSpin(wxSpinEvent &event){
    (*m_res) = wxString::Format("%d",GetValue()); //fixme: find the inttostr equivalent
    if ((*m_res) != m_startValue) m_owner->OnRenameAccept();
}
...
void wxEditSpinCtrl::OnKeyUp( wxKeyEvent &event )
{
    ...
    wxSize mySize = GetSize();
    int sx, sy;
    GetTextExtent( wxString::Format("%d",GetValue()) + _T("M"), &sx, &sy);
    if (myPos.x + sx > parentSize.x) sx = parentSize.x - myPos.x;
    if (mySize.x > sx) sx = mySize.x;
    SetSize(sx, -1);
...
void wxTreeListMainWindow::EditLabel (const wxTreeItemId& item, int column) {
    ...
    wxTreeListColumnInfo &colInfo = header_win->GetColumn(column);
	int pick_type = colInfo.GetPickType();
	switch(pick_type)
	{
		...
		case wxTR_COLUMN_SPIN:
			{
				...
				wxString columnValue = m_editItem->GetText(column);
				if (columnValue.Len() > 0 && columnValue.IsNumber()){
					wxEditSpinCtrl *spin = new wxEditSpinCtrl(this, -1, &m_renameAccept, &m_renameRes,
						this, m_editItem->GetText (column),
						wxPoint (x, y), wxSize (w, h), style);
					spin->SetFocus();
					spin->SetRange(colInfo.GetPickRangeMin(), colInfo.GetPickRangeMax());
				}
				return;
		}
	}
...
void wxTreeListMainWindow::SetItemChecked (const wxTreeItemId& item, const int column, bool checked){
	//TODO FIXME - this was unimplemented in the code we got from the author, need to figure out how to do it
}

wxTreeListItemType wxTreeListMainWindow::GetItemType (const wxTreeItemId& itemId, const int column)
{
    wxCHECK_MSG (itemId.IsOk(), wxItemTypeFirst, _T("invalid tree item") );
    wxTreeListItem *item = (wxTreeListItem*) itemId.m_pItem;
...
bool wxTreeListMainWindow::GetItemChecked(const wxTreeItemId& itemId, const int column){
	return false; //TODO FIXME - this was unimplemented in the code we got from the author, need to figure out how to do it
}
...
void wxTreeListCtrl::SetColumnPickRange(int column, int min, int max){
	wxTreeListColumnInfo &info = m_header_win->GetColumn(column);
	info.SetPickRangeMin(min);
	info.SetPickRangeMax(max);
}

int wxTreeListCtrl::GetColumnPickRangeMin(int column){
	return m_header_win->GetColumn(column).GetPickRangeMin();
}

int wxTreeListCtrl::GetColumnPickRangeMax(int column){
	return m_header_win->GetColumn(column).GetPickRangeMax();
}
